...
int					FSOUND_BufferSizeMs = 75;
HWAVEOUT			FSOUND_WaveOutHandle;
FSOUND_SoundBlock	FSOUND_MixBlock;

char * FMUSIC_GetPointerToOutputBuffer(void)
{	return FSOUND_MixBlock.data ;
}
...
void FSOUND_Software_Fill()
{
	...
	if(!totalblocks) totalblocks = FSOUND_BufferSize / FSOUND_BlockSize; 
	...
	{
		//void  			*ptr1;
		void * ptr1 = FSOUND_MixBlock.data + (mixpos << 2);
		FSOUND_MixerClipCopy_Float32(ptr1, mixbuffer, FSOUND_BlockSize);
		
		if(aditionalMix) // make an user aditional mix (to, by exemple add some sfx :)
			aditionalMix(ptr1, FSOUND_BlockSize<<1);

		// fill info for app who use minifmod
		curentMixBf  = ptr1;//curentOutBf ; curentMixBf += mixpos<<1;
		curentMixSz  = FSOUND_BlockSize<<1 ;

	//	curentOutBf  = (short*)FSOUND_MixBlock.data ; // set on buffer start
	//	curentOutSz  = (FSOUND_MixBlock.wavehdr.dwBufferLength)>>1 ; // get it in word
	}
...
//void playSound(LPCSTR pszSound)
//{	 PlaySound(pszSound, HMODULE hmod, DWORD fdwSound);
//}
int songIsPaused=0;
DWORD FSOUND_Software_DoubleBufferThread(LPDWORD lpdwParam)
{
	...
	while(!FSOUND_Software_Exit)
	{
		...
		{	int		cursorpos,cursorblock,prevblock;
			...
			if(FMUSIC_updateVolume(volumeUpdateSpeed)) songIsPaused = 2;

		} while(!songIsPaused && !FSOUND_Software_Exit);

		if(songIsPaused > 1)
		{	// volume is null, launch user change song callback
			onNoVolume();
			songIsPaused=0;
		}
