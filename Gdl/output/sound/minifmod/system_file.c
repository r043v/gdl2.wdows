...
//void			(*aditionalMix)(short*buffer, int size) = 0 ; // for user made it's own mix after minifmod

/* rov */

FMUSIC_MODULE *lastmodplay ;
int volumeUpdateSpeed ;

void(*aditionalMix)(short*buffer, int size) = 0; // callback to user own mix
void(*onNoVolume)(void) = 0;	// callback, launched when real volume reatched 0
void(*onSoundEnd)(void) = 0;	// user callback launched when song is finish..
void (*printlg)(const char * format, ...) = 0;
...
{		aditionalMix = userMix ;
		onNoVolume = onVolumeEnd ;
		onSoundEnd = onSongEnd ;
		printlg = log;
}
